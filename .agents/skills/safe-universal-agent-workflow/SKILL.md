...
- 修改后确认 31589 服务实际返回新文件，而不是只确认本地文件。
- 如果 127.0.0.1 和 10.26.6.8 返回不同，必须说明哪个是浏览器真实访问地址。

## 推荐检查

- `pwd` / `ls -la` 确认远端目录。
- `curl http://10.26.6.8:31589/...` 确认前端资源。
- `curl http://10.26.6.8:31835/api/health` 确认后端。
- `curl http://10.26.6.8:31835/api/llm/test` 只看 ok/provider/model，不输出 secrets。
